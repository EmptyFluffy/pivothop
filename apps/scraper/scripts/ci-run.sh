...
( cd apps/web && node scripts/company-desc.mjs; PAGE_GRACE_WRITE=1 npm run build && npm run --silent check:links && npm run --silent check:copy && node scripts/report-check.mjs ) || { echo "::error::web build, link gate or report gold set failed"; exit 2; }
