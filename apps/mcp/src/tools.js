 ...
 */
/**
 * Tool implementations, shared by BOTH front doors:
 *   - apps/mcp/src/index.js  — local stdio package (Claude Desktop, Cursor, IDEs)
 *   - apps/web/.../api/mcp   — remote HTTPS endpoint (claude.ai, ChatGPT, mobile)
 *
 * The only difference between them is where the JSON comes from, so that is the
 * one thing injected. Everything else — resolution, the honesty rule, citation
 * lines, attribution tags — lives here once. Two copies of this logic would
 * drift within a month, and the divergence would be invisible until one door
 * started answering differently from the other.
 */
let getJson = async () => { throw new Error('pivothop tools: no data source configured'); };
export function configure(fetcher) { getJson = fetcher; }

/* Public site root. Only used to BUILD the links we hand back — the data itself
   arrives through the injected fetcher, so each front door can source it
   however suits it. */
const BASE = 'https://www.pivothop.com';

/** Every URL we return is tagged, because otherwise a click arriving from a chat
 *  window is indistinguishable from someone typing the address, and "is anything
 *  actually using this?" becomes unanswerable. utm_source is what PostHog
 *  captures without configuration; the pages carry canonical tags to the clean
 *  URL so it does not fork the crawlable surface. */
const tag = (url) => `${url}${url.includes('?') ? '&' : '?'}utm_source=mcp`;

/** A human-readable citation on every answer.
 *
 *  Structured source/url fields are droppable — an assistant summarising into
 *  prose keeps the numbers and loses the attribution, so the measurement reaches
 *  the reader with none of our name on it. A sentence survives summarisation
 *  because it reads as part of the answer rather than as metadata. */
