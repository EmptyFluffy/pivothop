...
### SEO surfaces
- **Comparison pages rewritten around the question people type (2026-09-10).** First real GSC export: the compare family earns the impressions (PT vs RN 799, aerospace vs EE 695) at positions 5 to 15 with CTR under 1%, and the queries are "X vs Y salary" and "who gets paid more". Title now "pay, skills and which pays more", description leads with the pay verdict and the open-role count, short-answer band at the top, four freshest roles per side (JobsList), a two-board banner, linked shared skills and companies. The same export showed 36 compare URLs with 3,023 impressions returning 404 (the qualification rule had dropped them when a direction stopped scoring); restored as `PROVEN_PAIRS`, published on pay and boards with readiness stated as not scorable. CTA rules per template: docs/30.
- **Crawl and index policy fixed (2026-09-11, docs/35).** A public review found robots.txt (`Disallow: /jobs/*/*`, commit c4bb74858 of 2026-08-04, the edge-request diet) blocking five `/jobs/browse/<facet>` hubs that the sitemap asks Google to index, and the sitemap missing two newer facets (cities, languages). Now: `Allow: /jobs/browse/` for every agent; Googlebot and Bingbot groups that may fetch job detail pages so their `noindex, follow` is actually read; the generic group keeps the detail-page block for SEO tools and minor bots; `/j/` short links disallowed; sitemap facets derived from `FACETS`; `/direct` in the sitemap. New gate `check:robots` runs on the built robots.txt and sitemap.xml in the nightly. Job detail pages stay noindexed and carry no JobPosting; the four conditions to open a subset are in docs/35.
- **Direct postings, locked (2026-09-11).** Rows read from the employer's own system (`DIRECT_SOURCES` in JobCard: ATS feeds + the careers-page reader) show a "Direct" tag and keep company, logo and apply link locked behind `/direct` until the reader holds a plan (`html.ph-premium`, `components/PremiumGate.tsx`); title, location, pay and description stay open. Aggregator rows are never locked. The direct reader (`apps/scraper/src/sources/direct.js`) gained a provider adapter (Gemini Flash-Lite first, Haiku fallback), a metered per-run budget (`DIRECT_BUDGET_USD`) and schema.org JobPosting parsing before any model call.
- **Company pay + companies by country (2026-09-11), the two Himalayas families worth copying.** Every company page with 5+ postings stating pay (313 today) carries "What X pays, from its postings": 25th/median/75th band, posted pay by role (linked to salary pages) and by country, quartiles at 5+ stated and posted min/max at 3 to 4, nothing estimated. `/companies/in-<country>` (21 pages, same `/companies/[slug]` route, companies resolve first): the hub's ranked index scoped to one country with that country's own counts, an adaptive row floor (3, 5, 10, 20) that keeps the list under 250 rows and is stated on the page, freshest roles there, employer ask, four FAQ answers; linked from the hub's "By country" strip and from every `/jobs/in-<country>` page. Himalayas families NOT copied, on purpose: markets/industries (24k tag pages, the Zippia pattern), interview questions / job descriptions / resumes (templated prose at scale), talent directory (a business-model decision, docs/25 "join the pool").
- **Language pages + the Swiss block (2026-09-10).** `/jobs/<language>-speaking-in-<country>` (kind `lang-country`, floor 20 live roles, 29 pages at launch, own browse facet `/jobs/browse/languages`, board deep link `?lang=`): "English-speaking jobs in Switzerland" answered nowhere before, and GSC showed the demand landing on Swiss occupation pages. The requirement is the mined language gate, never the posting's own language. Every Switzerland-scoped category page now carries "Working in Switzerland, from these N postings": share open below 100% from the workload in titles, the language the postings state (only shown at 5+ stating one), the most-named locations, plus three FAQ answers in the same register. Swiss canton names (Vaud, Aargau, Ticino...) rejected as cities.
